...
class RkEmployeeRules:
    onDutyTm   = offDutyTm = onDutyLate60mTm = offDutyEarly60mTm = overTimeTm = 0
    offDutySTR = ''
    name = ''

    # 期限在本月分的特殊上班时间检查
    onBoundaryMonthFg = False
    personalRuleSettedFg = False
    boundaryDate = datetime.strptime('2099-1-1','%Y-%m-%d')

    office = ''
    def __init__(self, officeName = '', nam='', dtmMonthYear=datetime(year=2019,month=1,day=1)):
    
        if nam == '':
            raise RuntimeError('RkEmployeeRules: People name not defineded!')
        self.name = nam
        ...
        if self.office in OfficeRules_Dict.keys():
            ...
            raise RuntimeError('RkEmployeeRules: Branch name not defineded!')

        if self.name in ExceptionName_Dict.keys():
            rKey, self.boundaryDate = ExceptionName_Dict[self.name]
            self.personalRuleSettedFg = True
            #print (self.name, dtmMonthYear.year, self.boundaryDate.year, dtmMonthYear.month, self.boundaryDate.month)
            self.setRules(rKey)
            if ((dtmMonthYear.year == self.boundaryDate.year) and (dtmMonthYear.month == self.boundaryDate.month)):
                #print ('onBoundaryMonthFg = True')
                self.onBoundaryMonthFg = True
        else:
            self.personalRuleSettedFg = False
        
    def setRules(self, ruleKey=''):
        #print ('ruleKey=',ruleKey)
        self.onDutyTm, self.offDutyTm, self.offDutySTR, self.onDutyLate60mTm, self.offDutyEarly60mTm, self.overTimeTm = Rules_Dict[ruleKey]
    
    def checkAndSetPersonalRules(self, dtmMonthYear=datetime(year=2019,month=1,day=1)):
        # 正在年、月的边界月上，要每天检查
        
        if self.onBoundaryMonthFg == True:
             
            # personalRuleSettedFg is a toggle flag
            #print ('checkAndSetPersonalRules outside',self.name,dtmMonthYear.date(), self.boundaryDate.date())
            if ((self.personalRuleSettedFg == False) and (dtmMonthYear.date() <=  self.boundaryDate.date())):
                self.personalRuleSettedFg = True
                #print ('checkAndSetPersonalRules inside self.personalRuleSettedFg = True', self.name)
                self.setRules(ExceptionName_Dict[self.name][0])
            elif ((self.personalRuleSettedFg == True) and (dtmMonthYear.date() > self.boundaryDate.date())):
                self.personalRuleSettedFg = False
                #print ('self.personalRuleSettedFg = False', self.name)
                print (self.office)
                self.setRules(OfficeRules_Dict[self.office])

    def addOvertimeStr(self, lstDtm, fstDtm):
        retStr = '加班'
        dltm = lstDtm - fstDtm
        #retStr += '从'+fstDtm.strftime(" %H:%M:%S")+'\n到'+lstDtm.strftime(" %H:%M:%S")
        effectiveMinutes = (dltm.seconds / 60) - (dltm.seconds % 60) + 2 # 预防打卡时间太接近整数点
        effectiveMinutes -= (effectiveMinutes % 30)
        retStr += ('共' + str(int(effectiveMinutes / 60)) + ':' + str(int(effectiveMinutes % 60)))
    ...
    def ifEarly(self, dtm): #return 时间+'正常', '早退', '早退超60分缺席', ’加班'
        ...
        if timeNum < self.offDutyEarly60mTm:
            ...
            retStr += self.addOvertimeStr(dtm, dtmStdOffDtm)
    ...
    def AttenTwice(self, fstDtm, lstDtm):
        self.checkAndSetPersonalRules(fstDtm)
        retStr = ''
        #print (fstDtm, lstDtm)
        ...
        if fstDtm.weekday() == 5 or fstDtm.weekday() == 6: #周六 or 周日
            ...
            retStr += self.addOvertimeStr(lstDtm, fstDtm) 
            retStr += '\n'
...
class RkAttenTableToFile:
    ...
    def buildOutputList(self, dataList=[], rf = pd.DataFrame()):
        ...
        for i in range(0, len(dataList)): #依人次计数
            ...
            emRules = RkEmployeeRules(self.officeStr,nam,self.curMonthDtm)
            ...
            for j in range(0, len(dList)): #依每人在当月的资料段计数
                ...
                if dCnt > 1:
                    ...
                    # bubble sort， 不知为什么 pandas.DataFrame.sort 对时间字串排序不起作用？
...
def open_filename():
    ...
    try:
        ...
        if filepath != '':
            retcode = RkAttenTableToFile(filepath)
    except:
        raise RuntimeError('open_filename error!', retcode)
...
window = tk.Tk()
#设置窗口名称
window.title((datetime.now()).strftime("睿控考勤数据处理转换工具%Y-%m-%d"))
...
filemenu.add_command(label='Open', command=open_filename)
