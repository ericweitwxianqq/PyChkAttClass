...
def readExcelAndConvert(rawFileName):
    ...
    #rf = rf.sort_values(by = ['姓名',dtmKeyStr], ascending = [True, True])
    ...
    outputFileNameStr = officeStr+'{0}年{1}月考勤数据导出表格'
    dtm = datetime.strptime(rf.iloc[0][dtmKeyStr], "%Y-%m-%d %H:%M:%S")
    ...
    for mjrIdx in range(0, len(rf)-1): 
        ...
        #print ('dtm+nxtDtm', dtm, nxtDtm, fstDay, nxtDay)
        #ans = input()

        #print ('BEFORE nCnt+ndCnt',kno,nxtKno, dtm, nxtDtm, fstDay, nxtDay, nCnt, ndCnt)
    ...
    for i in range(0, len(dataList)): #依人次计数
        ...
        for j in range(0, len(dList)): #依每人在当月的资料段计数
            ...
            fstDtm = datetime.strptime(rf.iloc[fstIdx][dtmKeyStr], "%Y-%m-%d %H:%M:%S")
            lstIdx = fstIdx + (dCnt - 1)
            
            lstDtm = datetime.strptime(rf.iloc[lstIdx][dtmKeyStr], "%Y-%m-%d %H:%M:%S") 
            ...
            if dCnt > 1:
                ...
                for i in range (fstIdx,fstIdx+dCnt):
                    dtm = datetime.strptime(rf.iloc[i][dtmKeyStr], "%Y-%m-%d %H:%M:%S")
